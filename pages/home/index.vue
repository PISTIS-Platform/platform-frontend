...
const generatePDF = () => {
    ...
    const docDefinition = {
        content: [
            ...
            { text: selected.value.amount.toFixed(2) + ' PC' },
            { text: t('transactions.amountToProvider'), style: 'subheading' },
            {
                text: R.isNil(selected.value.transactionFee)
                    ? `${selected.value.amount.toFixed(2)} PC`
                    : `${(selected.value.amount - selected.value.transactionFee).toFixed(2)} PC`,
            },
            { text: t('transactions.transactionFee'), style: 'subheading' },
            {
                text: R.isNil(selected.value.transactionFee)
                    ? 'None'
                    : `${selected.value.transactionFee.toFixed(2)} PC`,
            },
            ...
            { text: t('data.designer.price'), style: 'subheading' },
            {
                text: monetization.value?.is_free
                    ...
                    : monetization.value?.type === 'subscription'
                      ? monetization.value?.price.toFixed(2) + ' PC ' + monetization.value?.subscription_frequency
                      : monetization.value?.price.toFixed(2) + ' PC',
            },
            monetization.value?.type === 'subscription'
                ? {
            ...
            <div v-if="selected" class="w-full flex flex-col gap-4 text-sm">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <span>{{ selected.amount.toFixed(2) }} PC</span>
                    </div>
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <span
                            >{{ (selected.amount - selected.transactionFee).toFixed(2) }} PC /
                            {{ selected.transactionFee.toFixed(2) }} PC</span
                        >
                    </div>
                    ...
                    <div v-if="monetization?.type === 'subscription'" class="flex flex-col gap-1">
                        ...
                        <span>{{
                            monetization?.is_free
                                ...
                                : monetization?.price.toFixed(2) + ' PC ' + monetization?.subscription_frequency
                        }}</span>
                    </div>
                    ...
                    <div v-else class="flex flex-col gap-1">
                        ...
                        <span>{{
                            monetization?.is_free ? $t('data.designer.free') : monetization?.price.toFixed(2) + ' PC '
                        }}</span>
                    </div>

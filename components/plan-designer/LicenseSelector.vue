...
watch(
    ...
    () => {
        if (monetizationPropComputed.value.type === 'nft') {
            resetLicenseDetails(licenses.find((license) => license.code === LicenseCode.NFT));
        } else {
            if (!props.isFree) {
                resetLicenseDetails(licenses.find((license) => license.code === LicenseCode.PISTIS));
            } else {
...
const licenseSelections = computed(() =>
    props.isFree
        ...
        : props.monetizationDetails.type === 'nft'
          ? [licenses.find((license) => license.code === LicenseCode.NFT)]
          : [licenses.find((license) => license.code === LicenseCode.PISTIS)],
);
...
const resetLicenseDetails = (license: { code: string; label: string; description?: string } | undefined) => {
    ...
    if (license.code === LicenseCode.PISTIS) {
        ...
        licenseDetails.value = {
            ...
            nftLicenseText: licenses.find((license) => license.code === LicenseCode.NFT)!.description,
        };
    } else {
        licenseDetails.value = {
